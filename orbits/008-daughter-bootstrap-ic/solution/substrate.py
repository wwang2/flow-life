#!/usr/bin/env python3
"""
Heredity Morphogen Flow-Lenia Substrate -- Orbit 008
=====================================================

IDENTICAL to orbit 006 substrate -- no modifications to fission dynamics.
Only the class docstring and SubstrateVariant alias are updated for orbit 008.

Key design principle:
  The orbit 006 substrate gets daughters=3 from its chaotic fission dynamics.
  We preserve ALL of that exactly. The heredity improvement in orbit 008 comes
  from bootstrapping the IC FROM a daughter snapshot (see run.py), not from
...
class HeredityMorphogenSubstrate(BaseFlowLenia):
    """Two-kernel Flow-Lenia with RD morphogen for heredity -- orbit 008.

    Built on orbit 006's EXACT mechanics. No changes to dynamics.
    Heredity is improved via daughter-bootstrap IC discovery in run.py.
    """
    ...
    def get_default_params(self) -> dict:
        return {
            # -- Two-kernel Flow-Lenia (EXACT orbit 006 params) --
            "R_inner": 10,
            "mu_inner": 0.15,
            "sigma_inner": 0.017,
            "w_inner": -0.8,
            ...
            "dt": 0.2,
            "flow_strength": 0.05,
            # -- RD Morphogen (orbit 006 params) --
            "D_a": 0.01,
            "D_h": 0.6,             # Orbit 006 value
            "mu_a": 0.08,
            "mu_h": 0.10,
            "rho": 0.002,
            "dt_rd": 0.02,
            ...
            # -- Spot printing (same as orbit 006) --
            "spot_print_strength": 0.04,
    ...
    def apply_growth(self, state, flow, params):
        ...
        # Additive morphogen coupling (orbit 006)
        ...
        # SPOT PRINTING: light Turing texture on density for visual heredity
        spot_str = self._to_float(params.get("spot_print_strength", 0.04))
        ...
        # Growth noise for SSIM stochasticity (orbit 006)
