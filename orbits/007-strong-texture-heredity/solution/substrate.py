#!/usr/bin/env python3
"""
Strong Texture Heredity Flow-Lenia Substrate -- Orbit 007
==========================================================

Built directly on orbit 006's proven HeredityMorphogenSubstrate.
Primary change: spot_print_strength 0.04 → 0.25 to make Turing texture
visible at 64×64 render resolution for CLIP/DINOv2 heredity scoring.

Key insight from orbit 006:
  - replication=1.0 (fission works perfectly)
  - heredity=0.007 (daughters look like uniform blobs, CLIP cos_sim ≈ 0.25,
    below 0.5 threshold → zero credit)
  - Root cause: spot_print_strength=0.04 too weak; texture not visible at 64×64

Changes from orbit 006:
  1. spot_print_strength: 0.04 → 0.25 (primary fix)
  2. spot_print_threshold: 0.5 → 0.3 (lower threshold → more spots in density)
  3. D_h: 0.6 → 0.8, D_a: 0.01 → 0.015 (slightly adjusted wavelength for
     ~8-12 visible spots in 40px-diameter circle at 64×64)
  4. ALL fission/flow parameters preserved exactly from orbit 006

Design principle: fission is driven by large-scale density gradients
(R_inner/R_outer kernels), NOT fine Turing texture. Stronger spot printing
modulates density amplitude but doesn't change the spatial gradient structure
that drives fission — so fission should survive.
"""
...
class StrongTextureHereditySubstrate(BaseFlowLenia):
    """Two-kernel Flow-Lenia with strong RD morphogen texture for heredity -- orbit 007.

    Identical to orbit 006 except:
    - spot_print_strength = 0.25 (was 0.04) — strong enough to show at 64×64
    - spot_print_threshold = 0.3 (was 0.5) — lower threshold → more texture coverage
    - D_h = 0.8, D_a = 0.015 — slightly tuned Turing wavelength
    """
    ...
    def get_default_params(self) -> dict:
        return {
            # -- Two-kernel Flow-Lenia (EXACT orbit 005/006 params) --
            "R_inner": 10,
            "mu_inner": 0.15,
            "sigma_inner": 0.017,
            "w_inner": -0.8,
            ...
            "dt": 0.2,
            "flow_strength": 0.05,
            # -- RD Morphogen (tuned for visible texture at 64×64) --
            "D_a": 0.015,            # Slightly up from 0.01 for wavelength tuning
            "D_h": 0.8,              # Up from 0.6 for more pronounced Turing spots
            "mu_a": 0.08,            # Same as orbit 006
            "mu_h": 0.10,            # Same as orbit 006
            "rho": 0.002,            # Same as orbit 006
            "dt_rd": 0.02,
            "rd_substeps": 5,        # Same as orbit 006
            # -- Morphogen coupling (same as orbit 006) --
            "morph_coupling": 0.08,
            "morph_threshold": 0.4,
            "morph_seed_strength": 0.03,
            "flow_gate_strength": 0.05,
            # -- Spot printing (STRONGER -- orbit 007 primary change) --
            "spot_print_strength": 0.25,    # Was 0.04 — strong enough for 64×64 visibility
            "spot_print_threshold": 0.3,    # Was 0.5 — lower → more spots in density
            # -- Uncapped growth (EXACT orbit 006 params) --
    ...
    # -- Growth with morphogen + strong spot printing ----------------------
    ...
    def apply_growth(self, state, flow, params):
        ...
        # Additive morphogen coupling (orbit 005/006 unchanged)
        ...
        # STRONG SPOT PRINTING: Turing texture in density for visual heredity
        # 0.25 (vs 0.04 in orbit 006) to be visible at 64×64 render resolution
        # Lower threshold (0.3 vs 0.5) for more coverage per daughter
        spot_str = self._to_float(params.get("spot_print_strength", 0.25))
        if self.morphogen is not None and spot_str > 0:
            a = self.morphogen[0]
            a_norm = (a - a.min()) / (a.max() - a.min() + 1e-8)
            spot_thresh = self._to_float(params.get("spot_print_threshold", 0.3))
            density_mask = (field > 0.05).float()
            spot_boost = spot_str * (a_norm - spot_thresh) * density_mask
            growth = growth + spot_boost

        # Growth noise for SSIM stochasticity (orbit 005/006 unchanged)
    ...
    # -- Main update step (orbit 006 logic, unchanged) ----------------------
    ...
    def update_step(self, state, params):
        ...
        if self.morphogen is None:
            ...
            if self._last_mass > 1e-8:
                ...
                # Only detect DROPS in mass (context switch to smaller pattern)
                # Do NOT trigger on mass INCREASES (normal growth)
                if ratio < 0.5:
    ...
    def to(self, device):
        ...
        # Preserves fission dynamics when evaluator calls to() before tier4
...
SubstrateVariant = StrongTextureHereditySubstrate
